"""HelloWorldNew Feed Integration for Cortex XSOAR - Unit Tests file
...
demisto-sdk lint -i Packs/HelloWorldNew/Integrations/FeedHelloWorldNew
...
before each test to simulate the API calls to the FeedHelloWorldNew API (which is
...
from FeedHelloWorldNew import Client, get_indicators_command, fetch_indicators_command
...
def test_build_iterator(requests_mock):
    ...
    """
    with open('test_data/FeedHelloWorldNew_mock.txt', 'r') as file:
        response = file.read()
...
def test_get_indicators_command(mocker):
    ...
    human_readable = tableToMarkdown('Indicators from HelloWorldNew Feed:', indicators_list,
