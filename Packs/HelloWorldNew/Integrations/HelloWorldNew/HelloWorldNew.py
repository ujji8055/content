"""HelloWorldNew Integration for Cortex XSOAR (aka Demisto)
...
Example Design document for the this Integration (HelloWorldNew):
...
The HelloWorldNew API is a simple API that shows a realistic use case for an XSOAR
...
This function is used in conjunction of the HelloWorldNew Scan playbook that uses
...
Please check the HelloWorldNew Design Document referenced above for details about
...
and the prefix (i.e. ``HelloWorldNew.Alert``) is passed via the ``outputs_prefix``
argument.
...
the list of entities (i.e. in HelloWorldNew you want to update the status of an
existing ``HelloWorldNew.Alert`` in the context when you retrieve it, rather than
...
class Client(BaseClient):
    ...
    For this HelloWorldNew implementation, no special attributes defined
    """
    ...
    def search_alerts(self, alert_status: Optional[str], severity: Optional[str],
        ...
        """Searches for HelloWorldNew alerts using the '/get_alerts' API endpoint
        ...
        :return: list containing the found HelloWorldNew alerts as dicts
    ...
    def get_alert(self, alert_id: str) -> Dict[str, Any]:
        """Gets a specific HelloWorldNew alert by id
    ...
    def update_alert_status(self, alert_id: str, alert_status: str) -> Dict[str, Any]:
        """Changes the status of a specific HelloWorldNew alert
    ...
    def scan_start(self, hostname: str) -> Dict[str, Any]:
        """Starts a HelloWorldNew scan on a specific hostname
    ...
    def scan_status(self, scan_id: str) -> Dict[str, Any]:
        """Gets the status of a HelloWorldNew scan
    ...
    def scan_results(self, scan_id: str) -> Dict[str, Any]:
        """Gets the results of a HelloWorldNew scan
...
def parse_domain_date(domain_date: Union[List[str], str], date_format: str = '%Y-%m-%dT%H:%M:%S.000Z') -> Optional[str]:
    ...
    Converts the HelloWorldNew domain WHOIS date (YYYY-mm-dd HH:MM:SS) format
...
def convert_to_demisto_severity(severity: str) -> int:
    """Maps HelloWorldNew severity to Cortex XSOAR severity

    Converts the HelloWorldNew alert severity level ('Low', 'Medium',
    'High', 'Critical') to Cortex XSOAR incident severity (1 to 4)
    for mapping.

    :type severity: ``str``
    :param severity: severity as returned from the HelloWorldNew API (str)
...
def test_module(client: Client, first_fetch_time: int) -> str:
    ...
    :type client: ``Client``
    :param Client: HelloWorldNew client to use
...
def say_hello_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """helloworldNew-say-hello command: Returns Hello {somename}
    ...
    # INTEGRATION DEVELOPER TIP
    # In this case 'name' is an argument set in the HelloWorldNew.yml file as mandatory,
...
def fetch_incidents(client: Client, max_results: int, last_run: Dict[str, int],
    ...
    for alert in alerts:
        ...
        # convert it from the HelloWorldNew API response
        ...
        # "type") or custom fields (like "helloworldNewid") directly here in the
        ...
        incident = {
            ...
            # 'type': 'Hello WorldNew Alert',  # Map to a specific XSOAR incident Type
            'severity': convert_to_demisto_severity(alert.get('severity', 'Low')),
            # 'CustomFields': {  # Map specific XSOAR Custom Fields
            #     'helloworldNewid': alert.get('alert_id'),
            #     'helloworldNewstatus': alert.get('alert_status'),
            #     'helloworldNewtype': alert.get('alert_type')
            # }
        }
...
def ip_reputation_command(client: Client, args: Dict[str, Any], default_threshold: int) -> List[CommandResults]:
    ...
    for ip in ips:
        ...
        # HelloWorldNew score to XSOAR reputation mapping
        ...
        # parts: the vendor-specific context (HelloWorldNew), the standard-context
        # (IP) and the DBotScore.
        # More information:
        # https://xsoar.pan.dev/docs/integrations/context-and-outputs
        # https://xsoar.pan.dev/docs/integrations/context-standards
        # https://xsoar.pan.dev/docs/integrations/dbot
        # Also check the HelloWorldNew Design Document

        # Create the DBotScore structure first using the Common.DBotScore class.
        dbot_score = Common.DBotScore(
            indicator=ip,
            indicator_type=DBotScoreType.IP,
            integration_name='HelloWorldNew',
            score=score,
            malicious_description=f'Hello WorldNew returned reputation {reputation}'
        )
        ...
        # INTEGRATION DEVELOPER TIP
        # In the integration specific Context output (HelloWorldNew.IP) in this
        ...
        # INTEGRATION DEVELOPER TIP
        # The output key will be ``HelloWorldNew.IP``, using ``ip`` as the key field.
        # ``indicator`` is used to provide the context standard (IP)
        command_results.append(CommandResults(
            readable_output=readable_output,
            outputs_prefix='HelloWorldNew.IP',
...
def domain_reputation_command(client: Client, args: Dict[str, Any], default_threshold: int) -> List[CommandResults]:
    ...
    for domain in domains:
        ...
        # INTEGRATION DEVELOPER TIP
        # The output key will be ``HelloWorldNew.Domain``, using ``domain`` as the key
        # field.
        # ``indicator`` is used to provide the context standard (Domain)
        command_results.append(CommandResults(
            readable_output=readable_output,
            outputs_prefix='HelloWorldNew.Domain',
...
def search_alerts_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """helloworldNew-search-alerts command: Search alerts in HelloWorldNew
    ...
    return CommandResults(
        outputs_prefix='HelloWorldNew.Alert',
...
def get_alert_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """helloworldNew-get-alert command: Returns a HelloWorldNew alert
    ...
    readable_output = tableToMarkdown(f'HelloWorldNew Alert {alert_id}', alert)
...
def update_alert_status_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """helloworldNew-update-alert-status command: Changes the status of an alert

    Changes the status of a HelloWorldNew alert and returns the updated alert info
...
def scan_start_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """helloworldNew-start-scan command: Starts a HelloWorldNew scan
    ...
    return CommandResults(
        readable_output=readable_output,
        outputs_prefix='HelloWorldNew.Scan',
...
def scan_status_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """helloworldNew-scan-status command: Returns status for HelloWorldNew scans
...
def scan_results_command(client: Client, args: Dict[str, Any]) -> Union[Dict[str, Any], CommandResults, List[CommandResults]]:
    """helloworldNew-scan-results command: Returns results for a HelloWorldNew scan
